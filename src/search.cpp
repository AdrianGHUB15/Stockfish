...
static constexpr std::array<int, 16> lmrDivisor = {3637, 2787, 2761, 2939, 3171, 3347, 3147, 2762,
        ...
        // Dynamic EMA parameters for root move
            int Scale          = 32;
            int ChiNumerator   = 300;
            int ChiDenominator = 200;   // Chi = 3/2 = 1.5
            int MinWeight      = 12;  // 37.5% minimum weight
            int MaxWeight      = 24;  // 75% maximum weight

            TUNE(SetRange(-64, 64), Scale);
            TUNE(SetRange(-600, 600), ChiNumerator);
            TUNE(SetRange(-24, 24), MinWeight);
            TUNE(SetRange(-48, 48), MaxWeight);
...
bool Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth >= limits.depth))
    {
        ...
        if (limits.use_time_management() && !threads.stop && !mainThread->stopOnPonderhit)
        {
            ...
            timeReduction =
              std::clamp(interpolate(double(rootDepth - lastBestMoveDepth), 4.96, 18.79, 0.639, 1.712),
                         0.629, 1.544);

            double reduction = (1.468 + mainThread->previousTimeReduction) / (2.284 * timeReduction);
